...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql_insert = "INSERT INTO users (username, firstname, lastname, email, phone, password) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt_insert = $conn->prepare($sql_insert);
    $stmt_insert->bind_param("ssssss", $username, $firstname, $lastname, $email, $phone, $hashed_password);

    if ($stmt_insert->execute()) {
        header("Location: ../register.html?status=success&message=Registration+successful,+Welcome+" . urlencode($firstname));
    } else {
        // Capture error message from the query and show it in the URL
        $error_message = $stmt_insert->error;
        header("Location: ../register.html?status=error&message=Failed+to+register.+" . urlencode($error_message));
    }
